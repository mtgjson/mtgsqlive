...
$mysqli = new mysqli(DB_HOST, DB_ROOT_USER, DB_ROOT_PASSWORD);
if ($mysqli->connect_errno) {
  echo 'Failed to connect to MySQL: (' . $mysqli->connect_errorno . ") " . $mysqli->connect_error;
  exit();
}

$mysqli->query("DROP DATABASE IF EXISTS " . DB_NAME) or error_log("ERROR: Couldn't delete database");
$mysqli->query("CREATE DATABASE " . DB_NAME) or error_log("ERROR: Couldn't create database");
$logger->info("Database " . DB_NAME . " recreated");

# Use the json variable to load and link card tables.
#load_cards(json_data, mysqli);

$logger->info("Close our db connection and print success!");
$mysqli->close ();
echo "Database updated";

/*$stmt = $mysqli->prepare ($sql) or die ("Couldn't search for $searchq");
$stmt->bind_param ('s', $searchq) or die ("Could not search for $searchq");
$stmt->execute () or die ("Could not search for $searchq");
