...
def create_db(database_connection):
    ...
    cursor.execute(
        "create table luLayouts (layId integer PRIMARY KEY, layoutName text);"
    )
    cursor.execute("create table luColors (colId integer PRIMARY KEY, colorName text);")
    cursor.execute(
        "create table luColorIdentities (ciId integer PRIMARY KEY, colorIdentityName text);"
    )
    cursor.execute(
        "create table luSupertypes (suptId integer PRIMARY KEY, supertypeName text);"
    )
    cursor.execute("create table luTypes (tId integer PRIMARY KEY, typeName text);")
    cursor.execute(
        "create table luSubtypes (subtId integer PRIMARY KEY, subtypeName text);"
    )
    cursor.execute(
        "create table luRarities (rId integer PRIMARY KEY, rarityName text);"
    )
    cursor.execute(
        "create table luPrintings (pId integer PRIMARY KEY, printingName text);"
    )
    cursor.execute("create table luBorders (bId integer PRIMARY KEY, borderName text);")
    cursor.execute(
        "create table luLegalities (legId integer PRIMARY KEY, legalityName text);"
    )
    cursor.execute("create table luFormats (fId integer PRIMARY KEY, formatName text);")
    cursor.execute(
        "create table luSets (setId integer PRIMARY KEY, setName text, setCode text, setReleaseDate text);"
    )

    # Create links tables
    cursor.execute(
        "create table lnkCardColor (lccId integer PRIMARY KEY, cId integer, colId integer);"
    )
    cursor.execute(
        "create table lnkCardColorIdentity (lcciId integer PRIMARY KEY, cId integer, ciId integer);"
    )
    cursor.execute(
        "create table lnkCardSupertype (lcsuptId integer PRIMARY KEY, cId integer, suptId integer);"
    )
    cursor.execute(
        "create table lnkCardType (lctId integer PRIMARY KEY, cId integer, tId integer);"
    )
    cursor.execute(
        "create table lnkCardSubtype (lcsubtId integer PRIMARY KEY, cId integer, subtId integer);"
    )
    cursor.execute(
        "create table lnkCardPrinting (lcpId integer PRIMARY KEY, cId integer, pId integer);"
    )
    cursor.execute(
        "create table lnkLegalityFormat (llfId integer PRIMARY KEY, legId integer, fId integer);"
    )
    cursor.execute(
        "create table lnkCardLegFormat (lclfId integer PRIMARY KEY, cId integer, llfId integer);"
    )
...
def get_db_id(table_name, table_column_name, table_id_column, lookup_value, cursor):
    ...
    if table_name == "lnkLegalityFormat":
        ...
        legality_id = get_db_id(
            "luLegalities", "legalityName", "legId", legality_rule["legality"], cursor
        )
        format_id = get_db_id(
            "luFormats", "formatName", "fID", legality_rule["format"], cursor
        )
        lookup_query = (
            "SELECT llfId FROM lnkLegalityFormat WHERE legId = ? AND fId = ?;"
        )
        cursor.execute(lookup_query, (legality_id, format_id))
    else:
        lookup_query = "SELECT {0} FROM {1} WHERE {2} = ?;".format(
            table_id_column, table_name, table_column_name
        )
    ...
    if len(returned_values) == 1:
        ...
        if table_name == "lnkLegalityFormat":
            cursor.execute(
                "INSERT INTO lnkLegalityFormat (legId, fId) VALUES (?, ?);",
                (legality_id, format_id),
            )
        else:
            cursor.execute(
        ...
        print(
            "ERROR: Lookup table search returned an unexpected number of results terminating."
        )
        exit(3)
...
def load_cards(json_data, db_connection):
    ...
    for card_set_code in set_names:
        set_data = json_data[
            card_set_code
        ]  # All of the data for the set (I.e. SOI-x.json)

        card_set_name = set_data["name"]
        set_release_date = set_data["releaseDate"]
        cursor.execute(
            "INSERT INTO luSets (setName, setCode, setReleaseDate) VALUES (?, ?, ?);",
            (card_set_name, card_set_code, set_release_date),
        )
        set_db_id = cursor.lastrowid
        cards_in_set = set_data[
            "cards"
        ]  # Now iterate through the setCards for each card in the set

        for card_in_set in cards_in_set:
            card_id = get_value_from_key(card_in_set, "id")
            card_layout_id = get_db_id(
                "luLayouts",
                "layoutName",
                "layId",
                get_value_from_key(card_in_set, "layout"),
                cursor,
            )
            ...
            card_rarity_id = get_db_id(
                "luRarities",
                "rarityName",
                "rId",
                get_value_from_key(card_in_set, "rarity"),
                cursor,
            )
            ...
            lan_card_variations = json.dumps(
                get_value_from_key(card_in_set, "variations")
            )
            card_image_name = get_value_from_key(card_in_set, "imageName")
            card_watermark = get_value_from_key(card_in_set, "watermark")
            card_border_id = get_db_id(
                "luBorders",
                "borderName",
                "bId",
                get_value_from_key(card_in_set, "border"),
                cursor,
            )
            ...
            card_foreign_names = json.dumps(
                get_value_from_key(card_in_set, "foreignNames")
            )
            ...
            if card_legalities:
                for legality in card_legalities:
                    cursor.execute(
                        "INSERT INTO lnkCardLegFormat (cId, llfId) VALUES (?, ?);",
                        (
                            card_db_id,
                            get_db_id(
                                "lnkLegalityFormat",
            ...
            if card_printings:
                for printing in card_printings:
                    cursor.execute(
                        "INSERT INTO lnkCardPrinting (cId, pId) VALUES (?, ?);",
                        (
                            card_db_id,
                            get_db_id(
                                "luPrintings", "printingName", "pID", printing, cursor
                            ),
                        ),
                    )
            ...
            if card_colors:
                for color in card_colors:
                    cursor.execute(
                        "INSERT INTO lnkCardColor (cId, colId) VALUES (?, ?);",
                        (
                            card_db_id,
                            get_db_id("luColors", "colorName", "colId", color, cursor),
                        ),
                    )
            ...
            if card_color_identities:
                for colorIdentity in card_color_identities:
                    cursor.execute(
                        "INSERT INTO lnkCardColorIdentity (cId, ciId) VALUES (?, ?);",
                        (
                            card_db_id,
                            get_db_id(
                                "luColorIdentities",
            ...
            if card_supertypes:
                for supertype in card_supertypes:
                    cursor.execute(
                        "INSERT INTO lnkCardSupertype (cId, suptId) VALUES (?, ?);",
                        (
                            card_db_id,
                            get_db_id(
                                "luSupertypes",
            ...
            if card_types:
                for type in card_types:
                    cursor.execute(
                        "INSERT INTO lnkCardType (cId, tId) VALUES (?, ?);",
                        (
                            card_db_id,
                            get_db_id("luTypes", "typeName", "tId", type, cursor),
                        ),
                    )
            ...
            if card_subtypes:
                for subtype in card_subtypes:
                    cursor.execute(
                        "INSERT INTO lnkCardSubtype (cId, subtId) VALUES (?, ?);",
                        (
                            card_db_id,
                            get_db_id(
                                "luSubtypes", "subtypeName", "subtId", subtype, cursor
                            ),
                        ),
                    )
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: %s <database path> <json path> [append current database]"
            % sys.argv[0]
        )
        exit(1)
